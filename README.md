## 👩‍💻 About Me – Tabassum Fathima

🎓 MSc Data Science Student | 💡 Data Enthusiast | 📊 Python & NLP Practitioner

As an MSc Data Science student with a strong foundation in Python, NLP, and analytics, I’m passionate about transforming complex data into actionable insights. Whether it’s detecting sentiment in app reviews or forecasting sales trends, I focus on building solutions that create real-world impact.

---

### 🚀 Highlighted Projects

- 🟡 [**Sentiment Analysis – Livin' by Mandiri App Reviews**](https://github.com/Tabassumfathima583/Livin-by-Mandiri-App-Reviews)  
  Analyzed 10,000+ app reviews to identify sentiment trends, helping guide product feature improvements.  
  **Stack**: `Python`, `Pandas`, `NLTK`, `TextBlob`, `Seaborn`, `Matplotlib`, `WordCloud`

- ❤️ [**Heart Disease Prediction**](https://github.com/Tabassumfathima583/Heart-Disease)  
  Built and optimized ML models to predict heart disease, achieving strong accuracy through feature tuning.  
  **Stack**: `pandas`, `numpy`, `scikit-learn`, `seaborn`, `matplotlib`

- 🛍️ [**Retail Sales Analysis (SQL + Python)**](https://github.com/Tabassumfathima583/Retail-sales-using-python-sql)  
  Analyzed 73,000+ transactions to uncover sales patterns and generate business insights.  
  **Stack**: `Python`, `SQL`, `Pandas`, `Matplotlib`, `Power BI`

- 🌧️ [**USA Rainfall Prediction Dashboard**](https://github.com/Tabassumfathima583/USA-Rainfall-Prediction)  
  Combined weather data and visualized rainfall trends using interactive Power BI dashboards.  
  **Stack**: `Python`, `Power BI`, `Pandas`

- 📊 [**Time Series Forecasting**](https://github.com/Tabassumfathima583/Time-series)  
  Forecasted trends using ARIMA, SARIMA, and exponential smoothing on time-dependent datasets.  
  **Stack**: `Python`, `statsmodels`, `Pandas`, `Matplotlib`

- 🎬 [**IMDB TV Show Reviews Analysis**](https://github.com/Tabassumfathima583/IMDB-TV-Show-Reviews)  
  Performed sentiment analysis and text mining on user reviews to assess show reception.  
  **Stack**: `NLTK`, `TextBlob`, `Pandas`, `Matplotlib`

- 📈 [**700+ AI & Data Jobs Analysis**](https://github.com/Tabassumfathima583/700-JOBs-Data-of-AI-Data-Fields-2025)  
  Analyzed job postings to uncover trends in roles, required skills, and salaries.  
  **Stack**: `Python`, `Pandas`, `Matplotlib`, `Seaborn`

- 📰 [**Newspaper Churn Prediction**](https://github.com/Tabassumfathima583/Newspaper-churn)  
  Predicted churn using XGBoost with 84.8% accuracy. Included data cleaning, EDA, and model tuning.  
  **Stack**: `XGBoost`, `scikit-learn`, `Pandas`, `Seaborn`

- 🌍 [**Language Detection AI**](https://github.com/Tabassumfathima583/language-detection)  
  Trained a RidgeClassifier to detect language from input text with interactive visual outputs.  
  **Stack**: `Python`, `scikit-learn`, `Pandas`, `Plotly`

---

- 📈 [**Churn-Prediction-Stream-Work-Ltd**](https://github.com/Tabassumfathima583/Churn-Prediction-Stream-Work-Ltd)  
  A comprehensive Jupyter Notebook project predicting customer churn for *Stream Work Ltd* using feature engineering and ML modeling.

- 🧹 [**Green-Cart-Ltd-Data-Wrangling**](https://github.com/Tabassumfathima583/Green-Cart-Ltd-Data-Wrangling)  
  Data cleaning and preprocessing workflows for *Green Cart Ltd* to prepare datasets for deeper analysis and modeling.

- 📋 [**Customer-Sign-Up-Behavior-Data-Quality-Audit**](https://github.com/Tabassumfathima583/Customer-Sign-Up-Behavior-Data-Quality-Audit)  
  Performed a data quality audit on customer sign-up behavior data, identifying data issues and validation insights.

---

### 🧠 Core Skills & Tools

**Programming & Data Tools**: Python, SQL, Jupyter Notebook, VS Code, GitHub  
**Libraries & Frameworks**: Pandas, NumPy, Scikit-learn, XGBoost, statsmodels  
**Visualization Tools**: Power BI, Matplotlib, Seaborn, Plotly  
**NLP & Text Analysis**: NLTK, TextBlob, Hugging Face (in progress)

---

### 📌 Currently Exploring

- Advanced storytelling with Power BI dashboards  
- NLP with Hugging Face Transformers and embeddings  
- Building polished, narrative-driven data science portfolios  
- Streamlit for deploying interactive data apps

---

> _“Turning data into decisions through insights & visualization.”_

Let’s connect and explore opportunities to collaborate in data science and analytics!
